...
用纯 HTML 页面承载一切"能画出来"的内容的仓库：UI 原型、架构图、模块结构图、业务流程图、
状态流转图、设计说明……只要能用 HTML/SVG/CSS/JS 表达，都可以放进指定目录，推送后自动构建
发布，线上就能直接打开一个链接看效果 —— 不需要前端框架，不需要后端，不需要等排期。
...
- 一个"原型" = 一个独立的 HTML 页面（可以带自己的 CSS / JS / Mock 数据）—— **不局限于 UI
  界面**，架构图、模块结构图、业务流程图、状态流转图、设计说明页，只要能用 HTML 画出来，
  都算。
...
靠脑补理解"点了之后会发生什么"，沟通成本低很多。Tab / 步骤条 / 状态高亮这些"切换效果"
用几行原生 JS 就能实现，不需要额外的工具或框架。

### 不仅是 UI 原型 —— 架构图 / 流程图 / 状态图也可以放这里

现在主流的表达方式基本都能用 HTML 做到，所以这里说的"原型"不局限于产品界面。同一套目录
结构、同一套发布流程，也适合放：

- **架构图 / 模块结构图**：用 HTML + SVG 画节点和连线，点击节点展开细节或跳转说明，比一张
  静态 PNG 信息量大得多，还能随时改。
- **业务流程图 / 时序图**：配合 Tab、步骤条、逐步展开的动画，把"发生了什么、下一步是什么"
  直接演示出来，而不是靠一段文字描述。
- **状态流转图**：点击某个状态节点，高亮对应的迁移路径和触发条件，比静态状态机图更直观。
- **设计说明 / 技术方案页**：图文混排 + 可折叠区块，当成一份"点得开"的活文档，而不是一份
  只能上下滚动的 PDF。

做法和做 UI 原型完全一样：建个目录、写 `prototype.json` + `index.html`，推送后就是一个可以
直接分享的链接 —— 不需要额外买图表工具、也不用为了一张架构图专门开一次评审会。
