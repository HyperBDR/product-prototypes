...
1. **一个原型 = 一个独立目录，不能互相牵连。** 不要在自己的原型里引用别的原型目录下的
   文件，也不要顺手改了别的产品 / 原型的内容。这样任何一个原型才能被单独复制、删除、
   移动而不影响别人。完整的隔离规则以 **[AGENTS.md](AGENTS.md)** 为准 —— 人和 AI 工具
   （Claude Code / Codex 等）都按这份文件执行，避免大家做隔离审查时标准不一致。
2. **不要手改 `portal/prototypes.generated.json`。** 这是构建脚本自动生成的清单，手改了
